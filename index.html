...
<head>
    ...
    <title>Chantiers Modernes - Levage Sécurisé</title>
    
    <!-- 1. Chargement de React & ReactDOM -->
    <script crossorigin src="https://unpkg.com/react@18/umd/react.production.min.js"></script>
    <script crossorigin src="https://unpkg.com/react-dom@18/umd/react-dom.production.min.js"></script>
    
    <!-- 2. Chargement de Babel pour comprendre le JSX -->
    <script src="https://unpkg.com/@babel/standalone/babel.min.js"></script>
    
    <!-- 3. Chargement de Tailwind CSS -->
    <script src="https://cdn.tailwindcss.com"></script>

    <!-- Configuration Tailwind pour les couleurs -->
    <script>
        tailwind.config = {
            theme: {
                extend: {
...
<body>
    ...
    <script type="text/babel">
        ...
        // --- BIBLIOTHÈQUE D'ICÔNES (SVG Inline pour éviter les dépendances externes) ---
        const Icons = {
            ...
            Truck: (p) => <svg {...p} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="3" width="15" height="13" rx="2" ry="2"/><polyline points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>,
            Anchor: (p) => <svg {...p} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="5" r="3"/><line x1="12" y1="22" x2="12" y2="8"/><path d="M5 12H2a10 10 0 0 0 20 0h-3"/></svg>,
            ...
            Wind: (p) => <svg {...p} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17.7 7.7a2.5 2.5 0 1 1 1.8 4.3H2"/><path d="M9.6 4.6A2 2 0 1 1 11 8H2"/><path d="M12.6 19.4A2 2 0 1 0 14 16H2"/></svg>,
            ShieldAlert: (p) => <svg {...p} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>,
            ...
            ChevronUp: (p) => <svg {...p} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="18 15 12 9 6 15"/></svg>,
            Move: (p) => <svg {...p} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="5 9 2 12 5 15"/><polyline points="9 5 12 2 15 5"/><polyline points="15 19 12 22 9 19"/><polyline points="19 9 22 12 19 15"/><line x1="2" y1="12" x2="22" y2="12"/><line x1="12" y1="2" x2="12" y2="22"/></svg>,
            Ruler: (p) => <svg {...p} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21.3 15.3a2.4 2.4 0 0 1 0 3.4l-2.6 2.6a2.4 2.4 0 0 1-3.4 0L2.7 8.7a2.41 2.41 0 0 1 0-3.4l2.6-2.6a2.41 2.41 0 0 1 3.4 0l12.6 12.6z"/><line x1="14.5" y1="5.5" x2="18.5" y2="9.5"/><line x1="12.5" y1="7.5" x2="16.5" y2="11.5"/><line x1="10.5" y1="9.5" x2="14.5" y2="13.5"/><line x1="8.5" y1="11.5" x2="12.5" y2="15.5"/><line x1="6.5" y1="13.5" x2="10.5" y2="17.5"/><line x1="4.5" y1="15.5" x2="8.5" y2="19.5"/></svg>,
            Weight: (p) => <svg {...p} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="5" r="3"/><path d="M6.5 8a2 2 0 0 0-1.905 1.457l-1.652 4.131A5 5 0 0 0 7.635 21h8.73a5 5 0 0 0 4.692-7.412l-1.652-4.131A2 2 0 0 0 17.5 8H6.5z"/></svg>,
            ...
            FilePlus: (p) => <svg {...p} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/><line x1="12" y1="18" x2="12" y2="12"/><line x1="9" y1="15" x2="15" y2="15"/></svg>,
            ...
            Edit3: (p) => <svg {...p} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>,
            Pencil: (p) => <svg {...p} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>,
            Building: (p) => <svg {...p} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="2" width="16" height="20" rx="2" ry="2"/><line x1="9" y1="22" x2="9" y2="22"/><line x1="15" y1="22" x2="15" y2="22"/><line x1="12" y1="22" x2="12" y2="22"/><path d="M12 2v4"/><path d="M12 18v4"/><path d="M4 10h16"/><path d="M4 14h16"/></svg>,
        };
        ...
        // --- CONSTANTES ---
        const BRAND_BLUE = "#004e98";
        const BRAND_RED = "#d92e2e";
        ...
        const HomePage = ({ navigate }) => (
            <div className="flex flex-col items-center justify-center min-h-[80vh] bg-white p-6 animate-in">
                <div className="max-w-4xl w-full text-center space-y-12">
                    <h1 className="text-4xl md:text-5xl font-bold text-slate-800">
                        Bienvenue sur le portail <br/>
                        <div className="mt-6"><span className="text-brand-blue">Levage Sécurisé</span></div>
                    </h1>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-10">
                        <button onClick={() => navigate('determine')} className="group relative h-64 rounded-2xl bg-brand-blue text-white p-8 shadow-xl hover:shadow-2xl hover:scale-105 transition-all duration-300 flex flex-col items-center justify-center gap-4">
                        ...
                        <button onClick={() => navigate('verify')} className="group relative h-64 rounded-2xl bg-brand-blue text-white p-8 shadow-xl hover:shadow-2xl hover:scale-105 transition-all duration-300 flex flex-col items-center justify-center gap-4">
            ...
            return (
                <div className="max-w-6xl mx-auto p-6 grid grid-cols-1 md:grid-cols-2 gap-12 items-center min-h-[60vh]">
                    <div className="space-y-8 animate-in">
                        <h2 className="text-2xl font-bold text-brand-blue border-b pb-4 mb-6">Paramètres de la mission</h2>
                        <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 shadow-sm">
                            <label className="block text-lg font-semibold text-slate-700 mb-3">Masse de la charge à soulever</label>
                            <div className="flex gap-4">
                                <input type="number" value={mass || ''} onChange={(e) => setMass(parseFloat(e.target.value))} className="flex-1 p-4 text-xl border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-blue outline-none" placeholder="0"/>
                                <select value={unit} onChange={(e) => setUnit(e.target.value)} className="w-24 p-4 text-xl font-bold bg-white border border-slate-300 rounded-lg text-brand-blue"><option value="kg">kg</option><option value="t">t</option></select>
                            </div>
                        </div>
                        <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 shadow-sm">
                            <label className="block text-lg font-semibold text-slate-700 mb-3">Distance de prise en charge / pose (m)</label>
                            <input type="number" value={distance || ''} onChange={(e) => setDistance(parseFloat(e.target.value))} className="w-full p-4 text-xl border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-blue outline-none" placeholder="0"/>
                        </div>
                        <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 shadow-sm">
                            <label className="block text-lg font-semibold text-slate-700 mb-3">Surface disponible pour l'engin (m²)</label>
                            <input type="number" value={surface || ''} onChange={(e) => setSurface(parseFloat(e.target.value))} className="w-full p-4 text-xl border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-blue outline-none" placeholder="0"/>
                        </div>
                    </div>
                        ...
                        {!result ? (
                            <div className="relative w-64 h-64">
                                <svg className="w-full h-full transform -rotate-90">
                                    ...
                                    <circle cx="128" cy="128" r="120" stroke="currentColor" strokeWidth="15" fill="transparent" strokeDasharray={2 * Math.PI * 120} strokeDashoffset={2 * Math.PI * 120 - (progress / 100) * 2 * Math.PI * 120} className="text-brand-blue transition-all duration-300 ease-out" strokeLinecap="round"/>
                                </svg>
                                <div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-4xl font-bold text-brand-blue">{progress}%</span><span className="text-sm text-slate-400 mt-2 uppercase tracking-widest">Analyse</span></div>
                            </div>
                        ) : (
                            <div className="text-center animate-in">
                                <div className="w-64 h-64 rounded-full bg-brand-blue text-white flex flex-col items-center justify-center shadow-lg mb-6 mx-auto">
        ...
        const MachineCreator = ({ category, onSave, onCancel, initialData }) => {
            ...
            const addBoom = () => { setBooms([...booms, { id: Date.now(), len: (booms[booms.length-1]?.len || 0) + 10, dataStr: "" }]); };
            const updateBoom = (id, field, value) => { setBooms(booms.map(b => b.id === id ? { ...b, [field]: value } : b)); };
            const removeBoom = (id) => { if(booms.length > 1) setBooms(booms.filter(b => b.id !== id)); };
            ...
            const handleSave = () => {
                ...
                booms.forEach(b => {
                    ...
                    lines.forEach(line => {
                        ...
                        if(parts.length >= 2) {
                            ...
                            if(!isNaN(d) && !isNaN(l)) points.push({d, l});
                        }
                    });
                    if(points.length > 0) {
                ...
                if(boomLengths.length === 0) { alert("Ajoutez au moins une longueur de flèche avec des données valides."); return; }

                const finalMaxLoad = isTonnes ? maxLoad * 1000 : maxLoad;
                const newMachine = {
                    id: initialData ? initialData.id : Date.now(),
                    category, name: name || "Nouvelle Grue", type: "crane", mode: "multi_chart",
                    maxLoad: finalMaxLoad, maxReach, maxHeight, hasTelescoping: false,
                    boomLengths: boomLengths.sort((a,b) => a - b), charts
                };
                onSave(newMachine);
            };

            return (
                <div className="bg-white p-6 rounded-xl shadow-lg border border-brand-blue animate-in">
                    <h2 className="text-xl font-bold text-brand-blue mb-4 flex items-center gap-2"><Edit3 /> {initialData ? "Modifier l'engin" : "Créateur d'Engin"}</h2>
                    <div className="grid grid-cols-2 gap-4 mb-4">
                        <div><label className="text-xs font-bold text-slate-500 uppercase">Nom</label><input className="w-full border p-2 rounded" value={name} onChange={e=>setName(e.target.value)} placeholder="Ex: Grue 50t" /></div>
                        <div><label className="text-xs font-bold text-slate-500 uppercase">Charge Max ({isTonnes ? 't' : 'kg'})</label><input className="w-full border p-2 rounded" type="number" value={isNaN(maxLoad) ? '' : maxLoad} onChange={e=>setMaxLoad(parseFloat(e.target.value))} /></div>
                        <div><label className="text-xs font-bold text-slate-500 uppercase">Portée Max (m)</label><input className="w-full border p-2 rounded" type="number" value={isNaN(maxReach) ? '' : maxReach} onChange={e=>setMaxReach(parseFloat(e.target.value))} /></div>
                        <div><label className="text-xs font-bold text-slate-500 uppercase">Hauteur Max (m)</label><input className="w-full border p-2 rounded" type="number" value={isNaN(maxHeight) ? '' : maxHeight} onChange={e=>setMaxHeight(parseFloat(e.target.value))} /></div>
                    </div>
                            ...
                            {booms.map((b, idx) => (
                                <div key={b.id} className="bg-slate-50 p-3 rounded border border-slate-200">
                                    <div className="flex justify-between mb-2">
                                        <div className="flex items-center gap-2"><span className="text-xs font-bold bg-slate-200 px-2 py-1 rounded">#{idx+1}</span><input type="number" className="w-20 p-1 border rounded text-center font-bold" value={b.len} onChange={(e) => updateBoom(b.id, 'len', e.target.value)} /><span className="text-sm text-slate-600">mètres</span></div>
                                        <button onClick={() => removeBoom(b.id)} className="text-red-500 hover:bg-red-50 p-1 rounded"><Trash2 size={16}/></button>
                                    </div>
                                    <textarea className="w-full text-xs font-mono p-2 border rounded h-24" placeholder={`Format: Distance ; Charge\nEx: 3 ; ${isTonnes?'50':'2500'}`} value={b.dataStr} onChange={(e) => updateBoom(b.id, 'dataStr', e.target.value)} />
                                </div>
                            ))}
                        </div>
                        <button onClick={addBoom} className="mt-2 text-sm text-brand-blue font-bold flex items-center gap-1 hover:underline"><Plus size={16}/> Ajouter longueur</button>
                    </div>
                    <div className="flex gap-4 mt-6 pt-4 border-t border-slate-200">
                        ...
                        <button onClick={handleSave} className="flex-1 py-3 bg-brand-blue text-white font-bold rounded hover:bg-blue-800 flex justify-center items-center gap-2"><Save size={18}/> Enregistrer</button>
                    </div>
                </div>
            );
        };
        ...
        const VerifyPage = () => {
            ...
            const handleCreateOrUpdateMachine = (newMachine) => {
                if (machines.find(m => m.id === newMachine.id)) { setMachines(machines.map(m => m.id === newMachine.id ? newMachine : m)); } 
                else { setMachines([...machines, newMachine]); }
                setSelectedMachineId(newMachine.id);
                setShowCreator(false);
                setMachineToEdit(null);
            };

            const handleEditClick = () => { setMachineToEdit(machine); setShowCreator(true); };
            const handleCancelCreator = () => { setShowCreator(false); setMachineToEdit(null); };
                ...
                <div className="space-y-6">
                    <div className="flex justify-center gap-4 bg-slate-100 p-2 rounded-xl">
                        {[{id: 'telehandler', label: 'Engin Télescopique', icon: Truck},{id: 'mobile_crane', label: 'Grue Télescopique', icon: Move},{id: 'crawler_crane', label: 'Grue Treillis', icon: Anchor}].map(cat => (
                            <button key={cat.id} onClick={() => setCategory(cat.id)} className={`flex items-center gap-2 px-6 py-3 rounded-lg font-bold transition-all ${category === cat.id ? 'bg-brand-blue text-white shadow-md' : 'bg-white text-slate-600 hover:bg-slate-200'}`}>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                        <div className="lg:col-span-5 space-y-4">
                            <div className="bg-white p-5 rounded-xl shadow-sm border-l-4 border-brand-blue">
                                <h3 className="font-bold text-brand-blue mb-3">Sélection de la machine</h3>
                                <div className="flex gap-2 mb-4">
                                    ...
                                    {!showCreator && selectedMachineId && (
                                        <button onClick={handleEditClick} className="p-3 border border-slate-300 rounded-lg bg-white text-slate-600 hover:bg-slate-100" title="Modifier cet engin"><Pencil size={20}/></button>
                                    )}
                                </div>
                                {!showCreator && (
                                    <button onClick={() => { setMachineToEdit(null); setShowCreator(true); }} className="w-full py-3 border-2 border-dashed border-brand-blue text-brand-blue font-bold rounded-lg hover:bg-blue-50 transition-colors flex justify-center items-center gap-2">
                                        <Plus size={20}/> Créer un engin manuellement
                                    </button>
                                )}
                            </div>
                            {!showCreator && (
                                <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 space-y-4 animate-in">
                                    ...
                                    {machine && machine.mode === 'multi_chart' && (
                                        <div className="p-3 bg-slate-50 rounded border border-slate-200">
                                            ...
                                            <div className="flex flex-wrap gap-1">{machine.boomLengths.map(len => (<button key={len} onClick={() => setSelectedBoomLen(len)} className={`px-2 py-1 text-xs rounded border ${selectedBoomLen === len ? 'bg-slate-800 text-white' : 'bg-white'}`}>{len}</button>))}</div>
                                        </div>
                                    )}
                                    <div className="space-y-4">
                                        <div><div className="flex justify-between text-sm"><label>Masse (t)</label> <span className="font-bold text-brand-blue">{inputLoad/1000} t</span></div><input type="range" min="0" max={finalMassSliderMax} step="50" value={inputLoad} onChange={(e) => setInputLoad(parseInt(e.target.value))} className="w-full accent-brand-blue h-2 bg-slate-200 rounded cursor-pointer" /><div className="flex justify-between text-[10px] text-slate-400 mt-1"><span>0t</span><span>Max échelle : {Math.round(finalMassSliderMax/1000)}t</span></div></div>
                                        <div><div className="flex justify-between text-sm"><label>Portée (m)</label> <span className="font-bold text-brand-blue">{inputDist} m</span></div><input type="range" min="0" max={machine?.maxReach + 2} step="0.5" value={inputDist} onChange={(e) => setInputDist(parseFloat(e.target.value))} className="w-full accent-brand-blue h-2 bg-slate-200 rounded cursor-pointer" /></div>
                                        <div><div className="flex justify-between text-sm"><label>Hauteur Crochet (m)</label> <span className="font-bold text-brand-blue">{inputHeight} m</span></div><input type="range" min="0" max={machine?.maxHeight + 2} step="0.5" value={inputHeight} onChange={(e) => setInputHeight(parseFloat(e.target.value))} className="w-full accent-brand-blue h-2 bg-slate-200 rounded cursor-pointer" /></div>
                                    </div>
                                </div>
                            )}
                        </div>
                        <div className="lg:col-span-7 space-y-4">
                            {showCreator ? (
                                <MachineCreator key={category} category={category} onSave={handleCreateOrUpdateMachine} onCancel={handleCancelCreator} initialData={machineToEdit} />
                            ) : (
                                <>
                                    <div className={`p-6 rounded-xl shadow-md border-l-8 ${isSafe ? 'bg-green-50 border-green-500' : 'bg-red-50 border-red-500'} animate-in`}>
            ...
            return (
                <div className="min-h-screen">
                    ...
                    <main className="p-4 md:p-8">
                        ...
                        {page === 'determine' && <div className="animate-in"><button onClick={() => setPage('home')} className="mb-4 flex items-center text-slate-500 hover:text-brand-blue font-bold"><ArrowLeft size={20} className="mr-1"/> Retour Accueil</button><DeterminePage /></div>}
                        {page === 'verify' && <div className="animate-in"><button onClick={() => setPage('home')} className="mb-4 flex items-center text-slate-500 hover:text-brand-blue font-bold"><ArrowLeft size={20} className="mr-1"/> Retour Accueil</button><VerifyPage /></div>}
                    </main>
                </div>
            );
        };
